<?php 
      ...
      $query1 = db_query( $karyawan_tetap);
      $query2 = db_query( $karyawan_kontrak);
      $query3 = db_query( $karyawan_outsourcing);
      $query4 = db_query( $karyawan_training);
      $data1 = db_fetch_array($query1);
      $data2 = db_fetch_array($query2);
 ?>
 <!-- BEGIN CONTENT -->
                  ...
                  <div class="form-group">
                    ...
                    <div class="col-sm-2">
                      <select class="form-control" name="id_bagian" id="id_bagian" required="" onchange='showKab()'>
                        ...
                        <?php
                          $sql1 = db_query( "SELECT * FROM tbl_bagian");
                          while($data1=db_fetch_array($sql1)){
                        ?>
...
<?php 
              ...
              $nama_bagian  = db_query( "SELECT bagian FROM tbl_bagian WHERE id_bagian='$id_bagian'");
              $nama_bagian1 = db_fetch_array($nama_bagian);

              $nama_jabatan  = db_query( "SELECT jabatan FROM tbl_jabatan WHERE id_jabatan='$id_jabatan'");
              $nama_jabatan1 = db_fetch_array($nama_jabatan);

              //tetap
              $sql_tetap = db_query( "SELECT COUNT(id_status) AS jumlah_status FROM tbl_karyawan
                  WHERE id_bagian='$id_bagian' AND id_jabatan='$id_jabatan' 
                  AND id_status=1");
              $data_tetap = db_fetch_array($sql_tetap);
                 $data_tetap['jumlah_status'];

              //kontrak
              $sql_kontrak = db_query( "SELECT COUNT(id_status) AS jumlah_status FROM tbl_karyawan
                  WHERE id_bagian='$id_bagian' AND id_jabatan='$id_jabatan' 
                  AND id_status=2");
              $data_kontrak = db_fetch_array($sql_kontrak);
                 $data_kontrak['jumlah_status'];


              //HASIL
              //tetap
              $sql_tetap2 = db_query( "SELECT tbl_karyawan.id_karyawan,COUNT(tbl_karyawan.id_status) 
              ...
              $data_tetap2 = db_fetch_array($sql_tetap2);
                 $data_tetap2['jumlah_status'];

              //kontrak
              $sql_kontrak2 = db_query( "SELECT tbl_karyawan.id_karyawan,COUNT(tbl_karyawan.id_status) 
                AS jumlah_status,tbl_nilaiakhir.bobot
                FROM tbl_bagian,tbl_jabatan,tbl_status,tbl_nilaiakhir,tbl_karyawan
                WHERE tbl_nilaiakhir.id_karyawan=tbl_karyawan.id_karyawan
                AND tbl_karyawan.id_bagian=tbl_bagian.id_bagian
                ...
                AND tbl_nilaiakhir.bobot='A'");
              $data_kontrak2 = db_fetch_array($sql_kontrak2);
