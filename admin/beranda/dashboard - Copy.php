<?php 
      ...
      $query1 = db_query( $jumlah_emiten);
      $query4 = db_query( $jumlah_user);

      $data1 = db_fetch_array($query1);
      $data4 = db_fetch_array($query4);

      $dari_tahun     = isset($_POST['dari_tahun']) ? $_POST['dari_tahun'] : '';
      $sampai_tahun   = isset($_POST['sampai_tahun']) ? $_POST['sampai_tahun'] : '';
      $dari_perusahaan   = isset($_POST['dari_perusahaan']) ? $_POST['dari_perusahaan'] : '';
      $sampai_perusahaan   = isset($_POST['sampai_perusahaan']) ? $_POST['sampai_perusahaan'] : '';
 ?>
 <!-- BEGIN CONTENT -->
              ...
              <div class="grid-body">
                <form class="form-horizontal" role="form" action="" method="post" id="form_combo" name="form1">
                  ...
                  <div class="form-group">
                    ...
                    <div class="col-sm-3">
                      <select name="dari_perusahaan" class="form-control">
                          <?php 
                            if (empty($dari_perusahaan)){
                          ?>
                          <option value="">--Pilih--</option>
                          <?php } else { ?>
                            <option value="<?php echo $dari_perusahaan?>"><?php echo $dari_perusahaan?></option>
                            <option value=""></option>
                          <?php } ?>
                          <?php 
                            $sql_p1  = db_query( "SELECT nama_emiten FROM tbl_emiten");
                            while($data_p1 = db_fetch_assoc($sql_p1)){
                          ?>
                          <option value="<?php echo $data_p1['nama_emiten'];?>"><?php echo $data_p1['nama_emiten'];?></option>
                          <?php 
                            }
                          ?>
                      </select>
                  ...
                  <div class="form-group">
                    ...
                    <label class="col-sm-2 control-label">Dengan Perusahaan</label>
                    <div class="col-sm-3">
                      <select name="sampai_perusahaan" class="form-control">
                           <?php 
                            if (empty($sampai_perusahaan)){
                          ?>
                          <option value="">--Pilih--</option>
                          <?php } else { ?>
                            <option value="<?php echo $sampai_perusahaan?>"><?php echo $sampai_perusahaan?></option>
                            <option value=""></option>
                          <?php } ?>
                          <?php 
                            $sql_p2  = db_query( "SELECT nama_emiten FROM tbl_emiten");
                            while($data_p2 = db_fetch_assoc($sql_p2)){
                          ?>
                          <option value="<?php echo $data_p2['nama_emiten'];?>"><?php echo $data_p2['nama_emiten'];?></option>
                          <?php 
                            }
                          ?>
                      </select>
  ...
  $(function () {
    $('#container').highcharts({
        ...
        xAxis: {
            ...
            categories: [
                          <?php 
                            if (!empty($dari_tahun && $sampai_tahun)){
                            $sql3  = db_query( "SELECT nama_emiten FROM tbl_emiten,tbl_eva
                                                  WHERE tbl_eva.kode_emiten=tbl_emiten.kode_emiten
                                                  GROUP BY nama_emiten");
                            }
                            else if (!empty($dari_perusahaan && $sampai_perusahaan)){
                              $sql3  = db_query( "SELECT nama_emiten FROM tbl_emiten,tbl_eva
                                                  WHERE tbl_eva.kode_emiten=tbl_emiten.kode_emiten
                                                  AND tbl_emiten.nama_emiten 
                                                  IN ('$dari_perusahaan','$sampai_perusahaan')
                                                  GROUP BY tbl_eva.kode_emiten");
                            }
        ...
        <?php 
              if (!empty($dari_tahun)){
                $sql   = "SELECT * FROM tbl_eva 
                        ...
                        GROUP BY tahun";
              } else if (!empty($dari_perusahaan)){
                $sql   = "SELECT * FROM tbl_emiten,tbl_eva
                          WHERE tbl_eva.kode_emiten=tbl_emiten.kode_emiten
                          AND tbl_emiten.nama_emiten 
                          IN ('$dari_perusahaan','$sampai_perusahaan')
                          GROUP BY tbl_eva.tahun";
              } 
              $query = db_query(  $sql )  or die(mysqli_error($conn));
              ...
              $tahun=$ret['tahun'];
                  ...
                  { 
                      ...
                      data: [
                              <?php
                                if (!empty($dari_tahun)){
                                $sql = db_query( "SELECT * FROM tbl_eva WHERE tahun='$tahun'");
                                } else  if (!empty($dari_perusahaan)){
                                $sql = db_query( "SELECT * FROM tbl_emiten,tbl_eva
                          WHERE tbl_eva.kode_emiten=tbl_emiten.kode_emiten
                          AND tbl_emiten.nama_emiten 
                          IN ('$dari_perusahaan','$sampai_perusahaan')
                          GROUP BY tbl_eva.tahun='$tahun'");
                                }
                                while($datanya=db_fetch_assoc($sql)){
